						...
						<div class="col-md-12">
							<div class="card">
								...
								<form id="marketForm" name="marketForm" method="post">
									<strong class="card-title">등록된 상품 목록</strong>
								</div>
								...
								<table id="bootstrap-data-table"
									...
									<tbody>
										<c:forEach var="result" items="${list }" varStatus="status">
											...
											<td align="center">
												<img src="data:image/jpeg;base64,<c:out value='${result.mar_Img}'/>" width="50px" height="50px"></td>

												<td><c:out value="${result.mar_content }" /></td>
												<td><fmt:formatNumber value="${result.mar_price}"
														pattern="###,###,###" /> <%-- <c:out value="${result.mar_price }"/> --%>
							...
							<div>
								<a href='#' onClick='fn_list()'>상품목록</a> <a href='#'
									onClick='fn_write()'>상품등록</a>
...
function fn_list(){
    ...
    form.action = "<c:url value='/productList.do'/>";
    form.submit();
...
<script type="text/javascript">
      $(document).ready(function() {
		...
		function mem_view(m_num){
          
          var form = document.getElementById("adminForm");
          var url = "<c:url value='marketList.do' />";
          url = url + "?m_num=" + m_num;
